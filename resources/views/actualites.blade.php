...
@section('content')
    <div class="relative w-full h-[400px] md:h-[500px] overflow-hidden">
        <img src="{{ asset('img/event.webp') }}" alt="Actualités" class="w-full h-full object-cover" loading="eager" fetchpriority="high" decoding="async">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                ...
                <article class="flex flex-col bg-white rounded-[1.5rem] overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex flex-col flex-1 p-6 gap-4">
                        ...
                        @endif
                        @if($item->lieu || ($item->type === 'evenements' && $item->date_evenement))
                        <div class="flex items-center gap-2 text-sm text-gray-500">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            {{ $item->lieu }}
                            @if($item->type === 'evenements' && $item->date_evenement)
