...
@section('title', 'Nos Marques – Bières')

@section('content')
    <div class="relative w-full h-[400px] md:h-[500px] overflow-hidden">
        <img src="{{ asset('img/marque.jpg') }}" alt="Nos Marques – Bières" class="w-full h-full object-cover">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center text-white px-4">
            <nav class="flex items-center gap-2 text-sm md:text-base font-medium mb-4">
                ...
                <span class="text-bracongo font-bold text-lg">></span>
                <span class="opacity-90">Bières</span>
            </nav>
            <h1 class="text-4xl md:text-6xl font-bold tracking-tight text-center">Nos Bières</h1>
        </div>
    </div>
    ...
    <section class="container mx-auto px-4 py-16">
        <div class="flex justify-center mb-20">
            <div class="relative w-full max-w-2xl flex items-center">
                <input type="text" id="beer-search" placeholder="Taper un nom de bière"
        ...
        <div id="boissons-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16 max-w-7xl mx-auto">
            ...
            @empty
            <div class="col-span-4 text-center text-gray-400 py-12">Aucune bière disponible pour le moment.</div>
            @endforelse
        </div>

        <div id="no-result" class="hidden text-center text-gray-400 py-12">Aucune bière ne correspond à votre recherche.</div>
    </section>

    <script>
    document.addEventListener('DOMContentLoaded', function () {
        var input = document.getElementById('beer-search');
        if (!input) return;
